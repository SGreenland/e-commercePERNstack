...
export const Homepage = (props) => {
  ...
  useEffect(() => {
   useStore.setState({itemCount: JSON.parse(sessionStorage.getItem("cart")).state.cartItems.length})
  })
  ...
  function filterProds(e) {
    ...
    for (let i=0; i<e.target.value.length; i++) {
      useStore.setState({products: products.filter(prod => prod.name.match(e.target.value[i]) || prod.name.includes(e.target.value[i].toUpperCase()))})
      }

    if (e.key === "Backspace") {
      for (let i=0; i<e.target.value.length; i++) {
        useStore.setState({products: productRef.current.filter(prod => prod.name.match(e.target.value[i]) || prod.name.includes(e.target.value[i].toUpperCase()))})
        }
    }
    ...
    <>
      <div className="homepage-nav">
        <div className="logocontainer">
          ...
          <picture>
            {/* <source media="(min-width:450px)" srcset={logo}/>
            <source srcset={logo_small}/> */}
    ...
    try {
      ...
      if (response.ok) {
       ...
       useStore.getState().greyOut();
        ...
        alertBox.style.display = "flex";
        alertBox.innerHTML = `${error.constraint === "unique_email" && "Email already registered."}<button onclick = window.location.reload()>Try again</button>`;
        useStore.getState().greyOut();
